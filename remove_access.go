...
func main() {
	...
	grantee := common.HexToAddress(os.Args[2])
	block := os.Args[3]
	if len(block) > 32 {
		block_1 := [32]byte{}
		block_2 := [32]byte{}
		...
		transaction, err := instance.RemoveAccessMultiple(
			auth,
			sender,
			grantee,
		...
		if err != nil {
			fmt.Println("Remove Access Operation Failed!")
			return
		}
		fmt.Println("Remove Access Successful.")
		fmt.Println("Transaction ID: ", transaction.Hash().Hex())
	} else {
		block_1 := [32]byte{}
		copy(block_1[:], []byte(block))
		transaction, err := instance.RemoveAccess(
			auth,
			sender,
			grantee,
			block_1,
		...
		if err != nil {
			fmt.Println("Remove Access Failed!")
			return
		}
		fmt.Println("Remove Access Successful.")
		fmt.Println("Transaction ID: ", transaction.Hash().Hex())
