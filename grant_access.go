...
func main() {
	...
	if len(block) > 32 {
		...
		transaction, err := instance.GrantAccessMultiple(
			auth,
			sender,
			grantee,
		...
		if err != nil {
			fmt.Println("Grant Operation Failed!")
			return
		}
		fmt.Println("Grant Operation Successful.")
		fmt.Println("Transaction ID: ", transaction.Hash().Hex())
	} else {
		block_1 := [32]byte{}
		copy(block_1[:], []byte(block))
		transaction, err := instance.GrantAccess(
			auth,
			sender,
			grantee,
			block_1,
